...
var MongoDatabank = function(params) {
    ...
    bank.incrBy = function(type, id, n, callback) {
        ...
        getCollection(type, function(err, coll) {
            if (err) {
                ...
                debug("Incrementing type " + type + " id " + id + " by " + n);
                coll.update({_id: id}, {"$inc": {"_v": n}, "$set": {"_s": true}}, {upsert: true, multi: false}, function(err) {
                    debug("err = " + err);
                    if (err) {
                        callback(err, null);
                    } else {
    ...
    bank.truncate = function(type, id, length, callback) {
        ...
        getCollection(type, function(err, coll) {
            if (err) {
                ...
                debug("truncating  type " + type + " and id " + id + " to " + length);
                coll.update({_id: id}, {"$push": {"_v": {"$each": [], "$slice": length}}, "$set": {"_s": true}}, {upsert: false, multi: false}, function(err, results) {
                    results = JSON.parse(JSON.stringify(results));
                    debug("err = " + err);
                    debug("results = " + JSON.stringify(results));
                    if (results) {
    ...
    bank.slice = function(type, id, begin, end, callback) {
        ...
        getCollection(type, function(err, coll) {
            if (err) {
                ...
                debug("getting slice for type " + type + " and id " + id + " with begin " + begin + " and end " + end);
                coll.findOne({_id: id}, {"_v": {"$slice": [begin, end - begin]}}, function(err, rec) {
                    debug("err = " + err);
    ...
    bank.item = function(type, id, index, callback) {
        this.slice(type, id, index, index + 1, function(err, value) {
            if (err) {
                callback(err);
            } else if (value.length == 0) {
                callback(new NoSuchItemError(type, id, index));
            } else {
    ...
    bank.length = function(type, id, callback) {
        ...
        getCollection(type, function(err, coll) {
            if (err) {
                ...
                debug("getting length for type " + type + " and id " + id);
                coll.aggregate([{"$match": {"_id": id}}, {"$project": {"_len": { $size: "$_v" }}}], function(err, rec) {
                    debug("err = " + err);
                    debug(JSON.stringify(rec));
                    var value;
                    if (err) {
