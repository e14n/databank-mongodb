...
var MongoDatabank = function(params) {
    ...
    bank.append = function(type, id, toAppend, callback) {
        ...
        getCollection(type, function(err, coll) {
            if (err) {
                ...
                debug("appending " + JSON.stringify(toAppend) + " to type " + type + " and id " + id);
    ...
    bank.prepend = function(type, id, toPrepend, callback) {
        ...
        getCollection(type, function(err, coll) {
            if (err) {
                ...
                debug("prepending " + JSON.stringify(toPrepend) + " to type " + type + " and id " + id);
                coll.update({_id: id}, {"$push": {"_v": {"$each": [toPrepend], "$position": 0}}, "$set": {"_s": true}}, {upsert: true, multi: false}, function(err) {
                    debug("err = " + err);
                    if (err) {
                        callback(err);
                    } else {
    ...
    bank.remove = function(type, id, toRemove, callback) {
        ...
        getCollection(type, function(err, coll) {
            if (err) {
                ...
                debug("removing " + JSON.stringify(toRemove) + " from type " + type + " and id " + id);
                coll.update({_id: id}, {"$pull": {"_v": toRemove}, "$set": {"_s": true}}, {upsert: false, multi: false}, function(err, results) {
                    ...
                    if (err) {
                        ...
                        callback(new NoSuchThingError(type, id));
                    } else if (results && results.nModified === 0) {
                        callback(new NoSuchItemError(type, id, toRemove));
                    } else if (results && results.n == 1 && results.nModified == 1) {
                        callback(null);
                    } else {
                        callback(new Error("Unexpected results from mongodb"));
                    }
                });
            }
        });
