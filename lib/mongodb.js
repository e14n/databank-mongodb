...
MongoDatabank.servers = {};
MongoDatabank.getServer = function(host, port) {
    var server;
    if (MongoDatabank.servers[host] && MongoDatabank.servers[host][port]) {
        return MongoDatabank.servers[host][port];
    } else {
        server = new Server(host, port, {auto_reconnect: true});
        if (!MongoDatabank.servers[host]) {
            MongoDatabank.servers[host] = {};
        }
        MongoDatabank.servers[host][port] = server;
        return server;
    }
};
...
MongoDatabank.prototype.connect = function(params, callback) {
    ...
    server = MongoDatabank.getServer(bank.host, bank.port);
